    ...
    def retry_with_backoff(self, operation: Callable[..., T], *args: Any, **kwargs: Any) -> T:
        ...
        for attempt in range(self.retry_config.max_attempts):
            try:
                ...
                self.logger.warning(f"Operation failed (attempt {attempt + 1}/{self.retry_config.max_attempts}): {e}. Retrying in {delay:.2f}s")
                time.sleep(delay)
            ...
            def wrapper(*args: Any, **kwargs: Any) -> T:
                ...
                for attempt in range(max_retry):
                    try:
                        ...
                        self.logger.warning(f"Function {func.__name__} failed (attempt {attempt + 1}/{max_retry}): {e}. Retrying in {retry_delay:.2f}s")
